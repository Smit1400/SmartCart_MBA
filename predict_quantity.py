...
def predict_quantity(data):
    ...
    with st.expander("🚀 Discover the future of shopping with our cutting-edge Product Forecast Tool! 🛍️"):
        st.text("""
        ...
        if predict_productName != "Select":
            ...
            if st.button("Predict"):
                ...
                with st.spinner("Running the algorithm...."):
                    quantity = predict(data, str(date), predict_country, predict_productName)
                    st.text("Successfully Predicted 📈")
                st.text("🌟 Get ready to meet demand with precision: ")
                st.success(f"You're set to sell around :red[_{quantity}_] of these gems in the next 7 days! 🚀")
