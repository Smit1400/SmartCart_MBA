...
class CollaborativeFiltering():
    def __init__(self, data):
        ...
        self.user_item_matrix = data.pivot_table(index='CustomerNo', columns='ProductNo', values='Quantity', fill_value=0)
    ...
    def recommend_products_for_user(self, user_id, top_n=5):
        ...
        similar_users = self.user_similarity_df[user_id].sort_values(ascending=False).iloc[1:top_n+1].index
        
        # Get the products bought by the similar users
        products_bought_by_similar_users = self.user_item_matrix.loc[similar_users].sum().sort_values(ascending=False)
        ...
        recommended_products_name = self.data[self.data['ProductNo'].isin(list(recommended_products[:top_n]))]['ProductName'].unique()
